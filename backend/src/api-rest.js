...
app.get('/estadisticas/usuarios', async (req, res) => {
  ...
  try {
    ...
    const query = `
      ...
      WHERE rl.fecha_ingreso >= $1 AT TIME ZONE 'UTC+0' AND rl.fecha_ingreso < $2 AT TIME ZONE 'UTC+0'
  ...
  try {
    ...
    const query = `
      WITH regp_liq_det AS (
        ...
        SELECT liquidacion, 
               ht.fecha_ingreso AT TIME ZONE 'UTC+0' AS fecha_ingreso, 
               ht.fecha_entrega AT TIME ZONE 'UTC+0' AS fecha_entrega, 
               ht.num_tramite, 
               COUNT(*) AS ntramites,
                ahp.end_time_ AT TIME ZONE 'UTC+0' AS end_time_,
               EXTRACT(EPOCH FROM (ahp.end_time_ - np.fecha_proceso)) / 60 AS duracion_min,
               SUM(CASE 
                  WHEN EXTRACT(EPOCH FROM (((ht.fecha_entrega::DATE)::TIMESTAMP AT TIME ZONE 'UTC+0' + INTERVAL '16 hours') - np.fecha_proceso)) < 0 
                   THEN 1 
                   ELSE 0 
                 END) AS fuera_cal
        ...
        WHERE liq.fecha_ingreso >= $1 AT TIME ZONE 'UTC+0' AND liq.fecha_ingreso < $2 AT TIME ZONE 'UTC+0'
      ...
      WHERE rl.fecha_ingreso >= $1 AT TIME ZONE 'UTC+0' AND rl.fecha_ingreso < $2 AT TIME ZONE 'UTC+0' AND ro.nombre = $3
...
app.get('/estadisticas/detalle_usuario', async (req, res) => {
 ...
 with npasos as (
	...
	select a.id_proceso, a.npasos, b.name_ as ultima_accion, b.start_time_ AT TIME ZONE 'UTC+0' AS fecha_proceso
	from n_tareas a
	left join act_hi_taskinst b on a.idd = b.id_
)
select ht.num_tramite, 
       ...
       np.fecha_proceso as fecha_proceso,
        ahp.end_time_ AT TIME ZONE 'UTC+0' AS fecha_fin, 
       liq.peso_tramite, 
...
app.get('/estadisticas/detalle_tramite', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const eventosQuery = `
        SELECT 
          aht.end_time_ AT TIME ZONE 'UTC+0' AS fecha_fin,  
      ...
      const cambiosQuery = `
        SELECT 
          rb.fecha_hora AT TIME ZONE 'UTC+0' AS fecha_hora, 
