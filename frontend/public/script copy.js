...
function drawTramitesChart() {
  ...
  if (!canvas) {
    console.error('Canvas tramitesChart no encontrado');
    return;
  }
  if (!tramitesChart && usersData.length > 0) {
    ...
    console.log('Datos para gráfico de trámites:', dataValues);
...
function populateCambiosTable(rows) {
  ...
  table += '</tbody></table>';
  console.log('cambiosTable element:', cambiosTable);
